...
def _redis():
    from backend.services.p1.for_redis import _get_client
    return _get_client()
...
def persist_state(state: dict):
    ...
    """
    from backend.services.p3.for_state import save_state
    cache_state(state)
...
def restore_state(thread_id: str) -> Optional[dict]:
    ...
    from backend.services.p3.for_state import load_state

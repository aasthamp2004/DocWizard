...
Flow:
  START
    ...
    → retrieve
    → check_sufficiency
        ↓ "answer"  → generate_answer → post_answer_check → ask_ticket_decision (if needed) → END
        ↓ "ask_ticket" → ask_create_ticket → user responds → route decision → create_ticket → END
        ↓ (user declines)                                                    → END
...
from backend.services.p3.for_state  import AssistantState, new_state, add_message
from backend.services.p3.for_memory import persist_state, restore_state
from backend.services.p3.for_nodes  import (
...
def _route_after_retrieve(state: dict) -> Literal["answer", "ask_ticket"]:
    result = check_sufficiency(state)

    if result in ["answer", "ask_ticket"]:
        return result
...
def _build_graph():
    ...
    class GraphState(TypedDict, total=False):
        ...
        _needs_ticket_decision:  bool
        _needs_ticket:           bool
        _waiting_for_ticket_decision: bool
        _similar_questions:      list
        _skip_normal_flow:       bool
        _user_accepted_ticket:   bool
        _user_declined_ticket:   bool

    g = StateGraph(GraphState)

    # Nodes
    g.add_node("check_ticket_decision", check_ticket_decision)
    g.add_node("classify_intent",    classify_intent)
    g.add_node("ask_clarification",  ask_clarification)
    g.add_node("retrieve",           retrieve)
    g.add_node("generate_answer",    generate_answer)
    g.add_node("post_answer_check",  post_answer_check)
    g.add_node("ask_create_ticket",  ask_create_ticket)
    g.add_node("create_ticket",      create_ticket_node)

    # Edges
    g.set_entry_point("check_ticket_decision")

    # Check ticket decision first
    def _route_ticket_check(state: dict) -> Literal["create_ticket", "classify_intent", "__end__"]:
        if state.get("_user_accepted_ticket"):
            return "create_ticket"
        elif state.get("_user_declined_ticket"):
            return "__end__"
    ...
    g.add_conditional_edges(
        ...
        _route_after_retrieve,
        {
            ...
            "ask_ticket": "ask_create_ticket",
        }
    )

    # After generate_answer, check if LLM said "I don't know" → maybe ask for ticket
    g.add_edge("generate_answer", "post_answer_check")
    g.add_conditional_edges(
        "post_answer_check",
        _route_after_answer_check,
        {
            "ask_ticket_decision": "ask_create_ticket",
            "__end__":             END,
        }
    )

    # After asking about ticket, the conversation ends — wait for user response on next turn
    g.add_edge("ask_create_ticket", END)
